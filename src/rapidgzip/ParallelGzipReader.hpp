...
public:
    ...
    /**
     * @brief Get the current position in the compressed (gzip) stream.
     * @return Compressed byte offset in the underlying file
     */
    [[nodiscard]] size_t
    tellCompressedPosition() const
    {
        if ( !m_sharedFileReader ) {
            return 0;
        }
        return m_sharedFileReader->tell();
    }
